                        ...
                        <div class="table-responsive">
                            <table class="table data-table-assign"  width="100%">
                                ...
                                @foreach($reviews as $review)
                                    <tr class="order-row">
                                        <td>{{++$loop->index}}</td>
                                        <td>{{$review->ProductItem->product_item_name}}</td>
                                        <td>{{$review->user->email}}</td>
                                        <td>{{Str::limit($review->review, 100)}}</td>
                                        <td>{{Str::limit($review->reply,100)}}</td>
                                        <td class="align-right">
                                            <button class="btn btn-outline-info btn-sm review-view"  data-id="{{$review->product_review_id}}" title="View"><i class="ti-eye"></i> Message</button>
                                            <button class="btn btn-outline-primary btn-sm replay-review"  data-id="{{$review->product_review_id}}" title="Edit"><i class="ti-pencil"></i> Reply</button>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
    ...
    <div class="modal fade" id="reply-modal" tabindex="-1" role="dialog" aria-labelledby="city-2" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="reply-form" action="{{route('admin.product.reviews.update')}}" method="post">
                    @csrf
                    <div class="modal-body">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#reload-review').on('click', '.review-view' , function () {
                ...
                $.ajax({
                    url:"{{url('admin/productreviews/view')}}/"+review_id,
                    method:"get",
                    data:{},
                    dataType: 'json',
                    success:function(data){
                        if(data.product_review_id)
                        {
                            $('#review-modal').find('#review-product').html(data.product_item.product_item_name);
            ...
            $('#reload-review').on('click', '.replay-review' , function () {
                let review_id =  $(this).attr('data-id');
                $.ajax({
                    url:"{{url('admin/productreviews/reply')}}/"+review_id,
                    method:"get",
                    data:{},
                    dataType: 'json',
                    success:function(data){
                        if(data.product_review_id)
                        {
                            $('#reply-modal').find('#reply-product').html(data.product_item.product_item_name);
                            $('#reply-modal').find('#replay-text').html(data.reply);
                            $('#reply-modal').find('#review-id').val(data.product_review_id);
